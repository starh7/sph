        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="valueList in spuSaleAttrList" :key="valueList.id">
                <dt class="title">选择{{ valueList.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{active:spuSaleAttrValue.isChecked === '1'}"
                  v-for="spuSaleAttrValue in valueList.spuSaleAttrValueList"
                  :key="spuSaleAttrValue.id"
                  @click="changeIndex(spuSaleAttrValue,valueList.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuCount" @change="changCount"/>
...
export default {
  ...
  methods:{
    ...
    async addToCar(){
      try {
        // 告诉后台我加入购物车
      await this.$store.dispatch('addToCart',{skuid:this.$route.params.skuid,skunum:this.skuCount})
      // 把数据存在了本地库方便存取
      sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
      // 进行页面跳转
      this.$router.push({path:'/addcartsuccess',query:{skuNum:this.skuCount}})
      } catch (error) {
        alert(error.message)
      }
    }
