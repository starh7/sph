        ...
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con5">
            ...
            <input
              ...
              class="itxt"
              :value="cart.skuNum"
              @change="handler('ipt', $event.target.value * 1, cart)"
            />
            <a
              href="javascript:void(0)"
              class="plus"
...
export default {
  ...
  mounted() {
    this.getData();
    // this.$store.dispatch('deletecarbyid',this.cartInfoList[0].skuId)
  },
  computed: {
  ...
  methods: {
    ...
    handler: _lodash.throttle(async function (type, skuNum, cart) {
      switch (type) {
        case "mins":
          skuNum = cart.skuNum <= 1 ? 0 : -1;
          break;
        case "plus":
          skuNum = 1;
          // await this.$store.dispatch('shopCarList',{skuid:cart.skuId,skunum:skuNum})
          break;
        case "ipt":
   ...
   async deleteAllCheck(){
    ...
    async changeAllChecked(event){
      try {
        await this.$store.dispatch('allCheckedChange',event.target.checked?'1':'0')
        this.getData()
      } catch (error) {
        alert(error.message)
      }
