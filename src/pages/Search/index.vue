  ...
  <div>
    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li class="with-x" v-show="searchList.categoryName">
              {{ searchList.categoryName }}<i @click="removeName">×</i>
            </li>
            <!-- 关键字 -->
        ...
        <!--selector-->
        <SearchSelector @tradmark="getTradmark" @attrInfo="getAttrInfo" />
...
export default {
  ...
  mounted() {
    this.getData();
    this.$on("tradmark", this.getTradmark);
    this.$on("attrInfo", this.getAttrInfo);
  },
  computed: {
    ...mapGetters(["goodsList"]),
    isOne() {
      // console.log("111",this.searchList.order.split(":")[0].indexOf(flag) != -1);
  ...
  methods: {
    getData() {
      this.$store.dispatch("searchList", this.searchList);
    },
    removeName() {
      // alert("111")
    ...
    getAttrInfo(attr, item) {
      let str = `${attr.attrId}:${item}:${attr.attrName}`;
      // 数组去重
      if (this.searchList.props.indexOf(str) == -1) {
        this.searchList.props.push(str);
      }
      this.getData();
    },
    removeAttr(index) {
    ...
    change(flag) {
      // alert('123')
      let index = this.searchList.order.split(":")[0];
      console.log("i", index);
      console.log("f", flag);
      let sort = this.searchList.order.split(":")[1];
      let newOrder = "";
      if (index == flag) {
        newOrder = `${flag}:${sort != "desc" ? "desc" : "asc"}`;
